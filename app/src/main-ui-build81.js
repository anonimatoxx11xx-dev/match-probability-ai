...
function openSearch() {
  ...
  overlay.innerHTML = `
    <div class="b81-modal">
      <div class="b81-modal-head">
        <b>🔎 Cerca partite</b>
        <button class="b81-close" type="button">×</button>
      </div>
  ...
  const filter = () => {
    ...
    result.innerHTML = query
      ? `<b>${visible}</b> partite trovate per “${query}”.`
      : 'Scrivi per filtrare le partite di oggi.';
  };
...
function openSettings() {
  ...
  overlay.innerHTML = `
    <div class="b81-modal">
      <div class="b81-modal-head">
        <b>⚙ Impostazioni vista</b>
        <button class="b81-close" type="button">×</button>
      </div>
      <div class="b81-setting">
        <span><b>Vista compatta</b><br><small>Più partite visibili sullo schermo</small></span>
        <button class="b81-toggle ${compact ? 'on' : ''}" data-compact type="button"></button>
      </div>
      <div class="b81-setting">
        <span><b>Animazioni</b><br><small>Transizioni e feedback touch</small></span>
        <button class="b81-toggle on" data-motion type="button"></button>
      </div>
    </div>`;
...
function expandCard(card) {
  ...
  if (existing) {
    existing.remove();
    card.classList.remove('b81-open');
    return;
  }

  const probabilities = [...card.querySelectorAll('.prob b')].map((node) => node.textContent.trim());
  const markets = [...card.querySelectorAll('.market-line button')]
    .slice(0, 3)
  ...
  panel.innerHTML = `
    ...
    <div class="b81-expand-wide">
      <b>Segnali verificati</b>
      <small>${markets.join(' · ') || 'Dati disponibili nella scheda'}</small>
    </div>`;
...
function updateBuildLabel() {
  ...
  nodes.forEach((node) => {
    if (node.nodeValue?.includes('BUILD 80')) {
      node.nodeValue = node.nodeValue.replaceAll('BUILD 80', 'BUILD 81');
    }
  });
}
...
function enhance() {
  ...
  if (localStorage.getItem('mp_compact') === '1') {
    document.body.classList.add('b81-compact');
  }

  const searchButton = root.querySelector('.header-icons button:nth-child(1)');
  if (searchButton && !searchButton.dataset.b81) {
    searchButton.dataset.b81 = '1';
    searchButton.setAttribute('aria-label', 'Cerca');
  ...
  if (settingsButton && !settingsButton.dataset.b81) {
    settingsButton.dataset.b81 = '1';
    settingsButton.setAttribute('aria-label', 'Impostazioni');
  ...
  root.querySelectorAll('.match-card').forEach((card) => {
    ...
    card.addEventListener('click', (event) => {
      ...
      expandCard(card);
      card.setAttribute('aria-expanded', card.classList.contains('b81-open') ? 'true' : 'false');
    });
  });
