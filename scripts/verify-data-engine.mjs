...
const supported=matches.filter(x=>(x.sofaIdHome&&x.sofaIdAway&&x.sofaEventId)||(x.espnIdHome&&x.espnIdAway&&x.espnEventId)).slice(0,5);
assert(supported.length>=3,'Not enough fixtures with stable provider IDs for analysis verification');
...
assert(analyzed.some(x=>x.h2hCount>0||x.verifiedStats>0),'No H2H or verified team statistics recovered');
...
console.log(`PASS: ${matches.length} fixtures + competitions + recent form + H2H/stats + real team statistics.`);
